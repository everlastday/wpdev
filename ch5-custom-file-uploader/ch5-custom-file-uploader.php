  <?php
    ...
    Description: Chapter 5 – Custom File Uploader
    Version: 0.1
    Author: WestSIDe
    ...
    */
    ...
    function ch5_cfu_register_meta_box() {
      add_meta_box( 'ch5_cfu_upload_file', 'Upload File', 'ch5_cfu_upload_meta_box', 'post', 'normal' );
      add_meta_box( 'ch5_cfu_upload_file', 'Upload File', 'ch5_cfu_upload_meta_box', 'page', 'normal' );
    }
  ...
  <table>
      ...
      <td>
        <?php
          ...
          if ( empty ( $attachment_data ) ) {
            ...
            echo '">' . basename($attachment_data['url']) . '</a>';
          }
        ?>
      </td>
    </tr>
    ...
    <tr>
      <td>Delete File</td>
      <td><input type="submit" name="deleteattachment" class="button-primary" id="deleteattachment" value="Delete Attachment" /></td>
    </tr>
  </table>
    <?php }
      ...
      if ( isset($_POST['deleteattachment'] ) ) {
        ...
        if( array_key_exists( 'uploadpdf', $_FILES ) && !$_FILES['uploadpdf']['error'] ) {
          ...
          if ( $pdf_file_type != 'pdf' ) {
            ...
            $upload_return = wp_upload_bits( $_FILES['uploadpdf']['name'], null, file_get_contents( $_FILES['uploadpdf']['tmp_name'] ) );
