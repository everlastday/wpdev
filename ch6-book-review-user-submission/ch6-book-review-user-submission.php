<?php
  ...
  Description: Book Review User Submission
  Version: 0.1
  Author: WestSIDe
  ...
  function ch6_brus_book_review_form () {
    ...
    <form method="post" id="addbookreview" action="">
      ...
      <?php if ( isset( $_GET[ 'addreviewmessage' ] ) && $_GET[ 'addreviewmessage' ] == 1 ) { ?>
        <div style="margin: 8px;border: 1px solid #ddd; background-color: #ff0;"> Thank for your submission!</div>
      <?php } ?>
    ...
    if ( wp_verify_nonce( $_POST[ 'br_user_form' ],
      ...
      if ( empty( $_POST[ 'book_review_captcha' ] ) ) {
        ...
        if ( isset( $_COOKIE[ 'Captcha' ] ) ) {
          ...
          if ( md5( 'HDBHAYYEJKPWIKJHDD' . $_REQUEST[ 'book_review_captcha' ] . $_SERVER[ 'REMOTE_ADDR' ] . $time ) != $hash ) {
