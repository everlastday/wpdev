...
// Send blog table prefix to table creation function
  ...
  function ch7bt_settings_menu () {
    add_options_page( 'Bug Tracker Data Management', 'Bug Tracker', 'manage_options', 'ch7bt-bug-tracker', 'ch7bt_config_page' );
  }
  ...
  function ch7bt_config_page () {
    ...
    <?php if ( empty( $_GET[ 'id' ] ) ) {
      ...
      $bug_items = $wpdb->get_results( $wpdb->prepare( $bug_query, $wpdb->get_blog_prefix() ), ARRAY_A );
    ...
    <?php
      ...
      ?>
      <form method="post" action="<?php echo admin_url( 'admin-post.php' ); ?>">
        <input type="hidden" name="action" value="save_ch7bt_bug" /> <input type="hidden" name="bug_id" value="<?php echo esc_attr( $bug_id ); ?>" />
        <!-- Adding security through hidden referrer field -->
        ...
        <table>
          <tr>
            ...
            <td><input type="text" name="bug_title" size="60" value="<?php echo esc_attr( $bug_data[ 'bug_title' ] ); ?>" /></td>
          </tr>
          <tr>
            <td>Description</td>
          ...
          <tr>
            <td>Version</td>
            <td><input type="text" name="bug_version" value="<?php echo esc_attr( $bug_data[ 'bug_version' ] ); ?>" /></td>
          </tr>
          <tr>
            <td>Status</td>
            <td>
  ...
  function process_ch7bt_bug() {
    ...
    if ( isset( $_POST['bug_id'] ) && $_POST['bug_id'] == 'new') {
      ...
      $wpdb->update( $wpdb->get_blog_prefix() . 'ch7_bug_data', $bug_data,
